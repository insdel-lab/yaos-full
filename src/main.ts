...
export default class VaultCrdtSyncPlugin extends Plugin {
	...
	private get syncScopeOptions() {
		return {
			syncConfigDir: this.settings.syncConfigDir,
			configIncludePatterns: parseExcludePatterns(this.settings.configIncludePatterns),
		};
	}

	private isMarkdownPathSyncable(path: string): boolean {
		return isMarkdownSyncable(path, this.excludePatterns, this.app.vault.configDir, this.syncScopeOptions);
	}

	private isBlobPathSyncable(path: string): boolean {
		return isBlobSyncable(path, this.excludePatterns, this.app.vault.configDir, this.syncScopeOptions);
	}
