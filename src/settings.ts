...
export interface VaultSyncSettings {
	...
	/** Allow syncing a safe allowlist inside the Obsidian config directory. */
	syncConfigDir: boolean;
	/** Comma-separated config-path prefixes allowed when config dir sync is enabled. */
...
export const DEFAULT_SETTINGS: VaultSyncSettings = {
	...
	excludePatterns: "",
	syncConfigDir: false,
	configIncludePatterns: ".obsidian/app.json, .obsidian/appearance.json, .obsidian/hotkeys.json, .obsidian/community-plugins.json",
	maxFileSizeKB: 2048,
...
export class VaultSyncSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(containerEl)
			.setName("Sync selected Obsidian settings")
			.setDesc("Allow a safe subset of files inside .obsidian/ to sync. YAOS plugin data, workspace.json, workspaces.json, and plugin folders remain excluded.")
			.addToggle((toggle) =>
				toggle
		...
		if (this.plugin.settings.syncConfigDir) {
			new Setting(containerEl)
				.setName("Allowed Obsidian settings paths")
				.setDesc("Comma-separated vault-relative prefixes inside .obsidian/. Example: .obsidian/app.json, .obsidian/appearance.json")
				.addText((text) =>
					text
						.setPlaceholder(".obsidian/app.json, .obsidian/appearance.json")
						.setValue(this.plugin.settings.configIncludePatterns)
						.onChange(async (value) => {
							this.plugin.settings.configIncludePatterns = value;
							await this.plugin.saveSettings();
						}),
				);
		}
